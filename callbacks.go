			...
			if !scope.HasError() {
				...
				if validatorErrors != nil {
					if errors, ok := validatorErrors.(govalidator.Errors); ok {
						for _, err := range flatValidatorError(errors) {
...
func flatValidatorError(validatorErrors govalidator.Errors) []govalidator.Error {
...
func RegisterCallBacks(db *gorm.DB) {
	...
	callback.Update().Before("gorn:before_update").Register("valudations:valudate", validate)
}
